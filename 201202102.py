...
class Application:
	...
	if fl==0:
		frame=LabelFrame(master,text="Wave1",bg="turquoise")
      		frame.place(width=300,relx=0.05,rely=0.13,anchor=NW)
	elif fl==1:
		frame=LabelFrame(master,text="Wave2",bg="turquoise")
      		frame.place(width=300,relx=0.39,rely=0.13,anchor=NW)
	else:
		frame=LabelFrame(master,text="Wave3",bg="turquoise")
	...
	#Pause Button	
	self.pause = Button(frame, text="Pause", command=a.pause,fg="red")
	...
	#Play Button
	self.plays = Button(frame, text="Play", command=self.play,fg="red")
	...
	#timescale Scale
	self.varc=DoubleVar()
	self.scale3=Scale(frame,label="Time Scaling",variable=self.varc,orient=HORIZONTAL,from_=0.00,to=8.00,resolution=0.025)
...
w=Label(root,text="Wave Mixer",width=10,height=3,font=35,bg="azure",fg="black")
w.pack()
f=Frame(root,height=1,width=1000,bg="black")
f.pack()
a=AudioFile()
...
playmix = Button(root, text="Play and Mix", command=mixing,fg="medium blue")
playmix.place(relx=0.175,rely=0.92,anchor=CENTER)
playmodulate = Button(root, text="Play and Modulate", command=modulation,fg="medium blue")
playmodulate.place(relx=0.515,rely=0.92,anchor=CENTER)
record = Button(root, text="Record", command=record,fg="medium blue")
record.place(relx=0.800,rely=0.92,anchor=CENTER)
playrecord = Button(root, text="Play Recording", command=playit,fg="medium blue")
